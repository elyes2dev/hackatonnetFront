...
@Component({
  selector: 'app-quizf-form',
  templateUrl: './quizf-form.component.html',
  styleUrls: ['./quizf-form.component.scss']
})
export class QuizfFormComponent implements OnInit {
  quizForm: FormGroup;
  isEditMode = false;
  workshopId!: number;
  quizId?: number;
  ...
  private saveQuestionsBatch(questions: any[], quizId: number): void {
    ...
    Promise.all(saveObservables.map((obs: Observable<any>) => obs.toPromise()))
      ...
      navigateBackToQuizList(): void {
        ...
        if (workshopId) {
          this.router.navigate([`/workshopsf/${workshopId}/quizzes`]);
        }
      }
}
