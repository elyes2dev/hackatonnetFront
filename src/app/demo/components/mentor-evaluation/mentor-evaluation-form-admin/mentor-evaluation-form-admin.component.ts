...
@Component({
  selector: 'app-mentor-evaluation-form-admin',
  templateUrl: './mentor-evaluation-form-admin.component.html',
  styleUrls: ['./mentor-evaluation-form-admin.component.scss'],
  providers: [MessageService]
  
})
export class MentorEvaluationFormAdminComponent implements OnInit {
  evaluation = {
    ...
    if (this.isEditMode && this.evaluation.id !== null) {
      ...
      this.evaluationService.updateEvaluation(this.evaluation.id, this.evaluation).subscribe({
        next: (res) => {
          ...
          setTimeout(() => this.router.navigate(['/mentor-evaluations']), 1500);
        },
  ...
  cancelEdit() {
    this.router.navigate(['/mentor-evaluations']);
  }
}